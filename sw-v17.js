const CACHE = 'habit-companion-v17';
const ASSETS = ['./', './index.html', './styles.css?v=17', './icon-picker.css?v=17', './db.js?v=17', './data-integrity-v14.js?v=17', './app.js?v=17', './week-sunday.js?v=17', './momentum-period.js?v=17', './icons-ui.js?v=17', './reminders-v12.js?v=17', './backup-v14.js?v=17', './auto-backup-v17.js?v=17', './manifest.webmanifest', './icon.svg'];
...
async function getLatestBackup() {
  try {
    const db = await openDB(BACKUP_DB, 1);
    if (!db.objectStoreNames.contains(BACKUP_STORE)) { db.close(); return null; }
    return await new Promise((resolve, reject) => {
...
async function backupResponse(url) {
  const snapshot = await getLatestBackup();
  if (!snapshot?.backup) return new Response('No automatic backup is available.', { status: 404 });
  const requested = decodeURIComponent(url.pathname.split('/').pop() || 'habit-companion-backup.json');
  const filename = requested.replace(/[^a-zA-Z0-9._-]/g, '-') || 'habit-companion-backup.json';
...
self.addEventListener('fetch', event => {
  ...
  if (url.pathname.includes('/__habit_backup__/')) {
    event.respondWith(backupResponse(url));
    return;
  }
  event.respondWith(
